...
m = [[8, 2, 22, 97, 38, 15, 0, 4, 0, 75, 4, 5, 7, 78, 52, 12, 5, 77, 91, 8],
     ...
     [22, 31, 16, 71, 51, 67, 63, 89, 41, 92, 36, 54, 22, 4, 4, 28, 66, 33, 13, 80],
     [24, 47, 32, 6, 99, 3, 45, 2, 44, 75, 33, 53, 78, 36, 84, 2, 35, 17, 12, 50],
     [32, 98, 81, 28, 64, 23, 67, 1, 26, 38, 4, 67, 59, 54, 7, 66, 18, 38, 64, 70],
...
for x in range(0, len(m[0])):
    m_t.append([row[x] for row in m])

# Rows
for row in m:
    result.append(max([row[i] * row[i + 1] * row[i + 2] * row[i + 3] for i in range(0, len(row) - 4)]))
...
for y in range(0, len(m) - 3):
    for x in range(0, len(m) - 3):
        top = max(top, m[y][x] * m[y + 1][x + 1] * m[y + 2][x + 2] * m[y + 3][x + 3])
